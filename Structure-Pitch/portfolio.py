...
with st.sidebar:
    ...
    if has_K:   K     = st.number_input("K — strike", value=55.00000, step=1.0, format="%.5f")
    if has_sig: sigma = st.number_input("σ — volatility", value=0.35000, step=0.00001, format="%.5f", min_value=0.00001)
...
with col_s:
    _s0_default = st.session_state.get("S0", 50.0)
